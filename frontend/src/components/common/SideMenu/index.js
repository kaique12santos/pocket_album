...
const FaSignOutAlt = (props) => <FontAwesome5 name="sign-out-alt" {...props} />;
...
const FaHome = (props) => <FontAwesome5 name="home" {...props} />;
const FaFlag = (props) => <FontAwesome5 name="flag" {...props} />;
const FaBookOpen = (props) => <FontAwesome5 name="book-open" {...props} />;
const FaMapMarkerAlt = (props) => <FontAwesome5 name="map-marker-alt" {...props} />; // <-- NOVO ÍCONE ADICIONADO
...
const menuItems = [
  ...
  { key: 'tourist-spots', label: 'Pontos Turísticos', Icon: FaMapMarkerAlt, target: 'tourist-spots' }, // <-- NOVA ROTA AQUI
  { key: 'album', label: 'Meu Album', Icon: FaBookOpen, target: 'album' }, // Adicionei o target que faltava
  { key: 'brasil', label: 'Brasil Penta', Icon: FaFlag, target: 'brasil' },
  { key: 'quiz', label: 'Quiz Craque', Icon: FaStar, target: 'quiz' }, // Adicionei o target que faltava
  { key: 'guess-player', label: 'Quem e esse jogador', Icon: FaQuestionCircle, target: 'who-is-that-player' }, // Ajustei para bater com o seu App.js
...
export default function SideMenu({ visible, onClose, onNavigate, onSignOut }) {
  ...
  const [username, setUsername] = useState('PLAYER'); // Estado inicial
      ...
      try {
        ...
        if (user) {
          ...
          if (data && data.username) {
            ...
            // Fallback caso o perfil não tenha username
